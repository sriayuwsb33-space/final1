...
export default function App() {
  ...
  const priceList = useMemo(() => {
    ...
    Object.keys(KADAR_DATA).forEach(k => {
      const base = settings.goldPrice * KADAR_DATA[k];
      const sell = Math.ceil((base * (1 + settings.margin / 100)) / 5000) * 5000;
      const buy = Math.floor((sell * (1 - settings.buyback / 100)) / 5000) * 5000;
      list[k] = { sell, buy };
    });
    return list;
  }, [settings]);
